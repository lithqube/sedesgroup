	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('must be at least of', 'pg_ml').' '.$val.' '.__('characters', 'pg_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('maximum', 'pg_ml').' '.$val.' '.__('characters allowed', 'pg_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('must be long', 'pg_ml').' '.$val.' '.__('characters', 'pg_ml');
				}
				return false;
			}	
			else {return true;}
